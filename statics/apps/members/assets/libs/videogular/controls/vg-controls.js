...
'use strict';
angular
    .module('com.2fdevs.videogular.plugins.controls', [])
    .run([
        '$templateCache',
        function ($templateCache) {
            $templateCache.put(
                'vg-templates/vg-controls',
...
angular
    ...
    .run([
        ...
        function ($templateCache) {
            $templateCache.put(
                'vg-templates/vg-fullscreen-button',
...
angular
    ...
    .run([
        ...
        function ($templateCache) {
            $templateCache.put(
                'vg-templates/vg-play-pause-button',
...
angular
    ...
    .run([
        ...
        function ($templateCache) {
            $templateCache.put(
                'vg-templates/vg-playback-button',
 ...
 *
 */
angular.module('com.2fdevs.videogular.plugins.controls').directive('vgScrubBarBuffer', [
    function () {
        return {
            restrict: 'E',
            require: '^videogular',
...
angular
    ...
    .run([
        ...
        function ($templateCache) {
            $templateCache.put(
                'vg-templates/vg-scrub-bar-cue-points',
                    ...
                    scope.updateCuePoints = function onUpdateCuePoints(cuePoints) {
                        ...
                        if (cuePoints) {
                            ...
                            for (var i = 0, l = cuePoints.length; i < l; i++) {
                                var end =
                                    cuePoints[i].timeLapse.end >= 0
                                        ? cuePoints[i].timeLapse.end
                                        : cuePoints[i].timeLapse.start + 1;
                                var cuePointDuration = (end - cuePoints[i].timeLapse.start) * 1000;
                                var position =
                                    (cuePoints[i].timeLapse.start * 100) / Math.round(API.totalTime / 1000) + '%';
                                var percentWidth = 0;
 ...
 *
 */
angular.module('com.2fdevs.videogular.plugins.controls').directive('vgScrubBarCurrentTime', [
    function () {
        return {
            restrict: 'E',
            require: '^videogular',
...
angular
    ...
    .run([
        ...
        function ($templateCache) {
            $templateCache.put(
                'vg-templates/vg-scrub-bar-thumbnails',
    ...
    .directive('vgScrubBarThumbnails', [
        'VG_UTILS',
        function (VG_UTILS) {
            return {
                restrict: 'E',
                            ...
                            if (scope.vgThumbnails) {
                                for (var i = 0, l = scope.vgThumbnails.length; i < l; i++) {
                                    ...
                                    if (th.timeLapse.end >= 0) {
                                        if (
                                            lapse.start >= th.timeLapse.start &&
                                            (lapse.end <= th.timeLapse.end || lapse.end <= th.timeLapse.start)
                                        ) {
                                            scope.thumbnails = {
                    ...
                    scope.onScrubBarMouseMove = function onScrubBarMouseMove(event) {
                        if (scope.vgThumbnails && scope.vgThumbnails.length) {
                            var second = Math.round(
                                (event.offsetX * API.mediaElement[0].duration) / slider.scrollWidth
                            );
                            var percentage = Math.round((second * 100) / (API.totalTime / 1000));
 ...
 *
 */
angular.module('com.2fdevs.videogular.plugins.controls').directive('vgTimeDisplay', [
    function () {
        return {
            require: '^videogular',
            restrict: 'E',
 ...
 *
 */
angular.module('com.2fdevs.videogular.plugins.controls').directive('vgVolume', [
    'VG_UTILS',
    function (VG_UTILS) {
        return {
            restrict: 'E',
